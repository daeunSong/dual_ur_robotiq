...
void DrawingInput::readDrawingFile() {
  ...
  std::ifstream txt(ros::package::getPath("drawing") + "/data/input/sbr/" + this->drawing_file_name_full);
  ...
  while(std::getline(txt, line)) {
    if(line == "End") { // stroke finished
      drawing_pose.position.z = 1.32 + 0.025;
      ...
      //TODO: hardcoded
      y = (stod(tempSplit[1])-0.5) * this->ratio * this->target_size;
      x = (stod(tempSplit[0])-0.5) * this->target_size - 0.55;
      drawing_pose.position.x = x;
      drawing_pose.position.y = y;
      drawing_pose.position.z = 1.32;//this->drawing_pose.position.z - 0.15;

      if (this->color == '0' || this->color == '2') // right
...
//      drawing_pose.orientation.x = 0.0;
//      drawing_pose.orientation.y = 0.0;
//      drawing_pose.orientation.z = -0.7071068;
//      drawing_pose.orientation.w = 0.7071068;
