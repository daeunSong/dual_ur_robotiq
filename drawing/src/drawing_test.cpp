...
/*
void DrawingManager::pick_tool(int i) {
  std::vector<double> tool_y = {0.255, -0.273, 0.455, -0.473};
  std::vector<double> tool_x = {-0.96, -0.98, 0.96, 0.98};
  ...
  std::vector<geometry_msgs::Pose> linear_path;
  geometry_msgs::Pose command_pose;
  command_pose.position.x = -0.8;
  command_pose.position.y = tool_y[i];
  command_pose.position.z = 1.32;
  command_pose.orientation.x = 0.0;
  command_pose.orientation.y = 1.0;
  command_pose.orientation.z = 0.0;
  command_pose.orientation.w = 0.0;
  linear_path.push_back(command_pose);
  command_pose.position.x = tool_x[i];
  linear_path.push_back(command_pose);
  if (i%2 == 0) this->rightArm->computeCartesianPath(linear_path, this->eef_step, this->jump_threshold, trajectory);
  else          this->leftArm->computeCartesianPath(linear_path, this->eef_step, this->jump_threshold, trajectory);
  my_plan.trajectory_ = trajectory;
  this->trajectory_pub.publish(trajectory);

  if (i%2 == 0) this->rightArm->execute(my_plan);
  else          this->leftArm->execute(my_plan);
  linear_path.clear();

  gripper_msg.data = 'h';
  if (i%2 == 0) this->gripper_pub_right.publish(gripper_msg);
  else          this->gripper_pub_left.publish(gripper_msg);

  command_pose.position.z = 0.4;
  linear_path.push_back(command_pose);
  if (i%2 == 0) this->rightArm->computeCartesianPath(linear_path, this->eef_step, this->jump_threshold, trajectory);
  else          this->leftArm->computeCartesianPath(linear_path, this->eef_step, this->jump_threshold, trajectory);
  my_plan.trajectory_ = trajectory;
...
//  dm.init_drawing_pose_l.orientation.w = 0.0;
  ...
  // y,m left // c,k right
//  dm.colors.push_back("0"); dm.colors.push_back("1");
//  dm.colors.push_back("2"); dm.colors.push_back("3");
  ...
  for (int i = 0; i < drawing.stroke_num; i ++) {
    // read right
    ...
    if (stroke_num_r == i){
      ...
      if (arm_num_prev != arm_num) {
        // move the left arm (previous arm)
      }

      // plan right arm
      arm_num_prev = arm_num;
