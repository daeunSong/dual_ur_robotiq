...
void DrawingManager::initPose (){
  ...
  this->arms->setJointValueTarget("left_shoulder_pan_joint", 81.48*D2R); //
  this->arms->setJointValueTarget("left_shoulder_lift_joint", -28.84*D2R); //
  this->arms->setJointValueTarget("left_elbow_joint", -73.21*D2R); //
  this->arms->setJointValueTarget("left_wrist_1_joint", -32.65*D2R); //
  this->arms->setJointValueTarget("left_wrist_2_joint", 96.25*D2R); //
  this->arms->setJointValueTarget("left_wrist_3_joint", -40.27*D2R);  //

  // [RIGHT]: init joint pose
  this->arms->setJointValueTarget("right_shoulder_pan_joint", -194.76*D2R); //
  this->arms->setJointValueTarget("right_shoulder_lift_joint", -173.06*D2R); //
  this->arms->setJointValueTarget("right_elbow_joint", 90.65*D2R); //
  this->arms->setJointValueTarget("right_wrist_1_joint", 186.71*D2R); //
  this->arms->setJointValueTarget("right_wrist_2_joint", 46.87*D2R); //
  this->arms->setJointValueTarget("right_wrist_3_joint", 22.96*D2R);  //
...
//  moveit::core::RobotState& kinematic_state = planning_scene.getCurrentStateNonConst();
  ...
  // [ARMS]: open the gripper
  gripper_msg.data = 'o';
  dm.gripper_pub_left.publish(gripper_msg);
  dm.gripper_pub_right.publish(gripper_msg);



  //-------------------------- grasping pose
  geometry_msgs::Pose sphere_l_init = dm.leftArm->getCurrentPose(dm.EE_LINK_L).pose;
  std::vector<geometry_msgs::Pose> linear_path;

  // [ARMS]: pinch the gripper
  gripper_msg.data = 'p';
  dm.gripper_pub_left.publish(gripper_msg);
  dm.gripper_pub_right.publish(gripper_msg);

  // [LEFT]: move down
  sphere_l_init.position.z -= 0.1;
  linear_path.push_back(sphere_l_init);
  ...
  // [LEFT]:: move to center ready pose
  geometry_msgs::Pose sphere_l_pose = dm.leftArm->getCurrentPose(dm.EE_LINK_L).pose;
  sphere_l_pose.position.x = -0.4;
  sphere_l_pose.position.y = -0.3;
  sphere_l_pose.position.z = 0.95;
  sphere_l_pose.orientation.x = 0.0;
  sphere_l_pose.orientation.y = 0.0;
  sphere_l_pose.orientation.z = 0.70710;
  sphere_l_pose.orientation.w = 0.70710;
  linear_path.push_back(sphere_l_pose);
  ...
  /*
  // [LEFT]:: move to center ready pose
  dm.leftArm->setStartStateToCurrentState();
  dm.leftArm->setJointValueTarget("left_shoulder_pan_joint", 195*D2R); //
  dm.leftArm->setJointValueTarget("left_shoulder_lift_joint", -173*D2R); //
  dm.leftArm->setJointValueTarget("left_elbow_joint", 91*D2R); //
  dm.leftArm->setJointValueTarget("left_wrist_1_joint", 187*D2R); //
  dm.leftArm->setJointValueTarget("left_wrist_2_joint", 47*D2R); //
  dm.leftArm->setJointValueTarget("left_wrist_3_joint", 23*D2R);  //
  success = (dm.leftArm->plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
...
*/
  ...
  // [LEFT]: move to center towards
  sphere_l_pose.position.y = -0.0;
  linear_path.push_back(sphere_l_pose);
...
//////////////////////////////////////////////////////PRB
  // [RIGHT]: move to center ready pose
  geometry_msgs::Pose sphere_r_pose = dm.leftArm->getCurrentPose(dm.EE_LINK_R).pose;
  sphere_r_pose.position.y = 0.2;
  sphere_r_pose.position.z = 0.95;
//  sphere_r_pose.orientation.x = 0.5;
//  sphere_r_pose.orientation.y = -0.5;
//  sphere_r_pose.orientation.z = -0.5;
//  sphere_r_pose.orientation.w = 0.5;
  linear_path.push_back(sphere_r_pose);
  ...
  // [RIGHT]: move to center toward
  sphere_r_pose.position.y = 0.0;
  linear_path.push_back(sphere_r_pose);
  fraction = dm.rightArm->computeCartesianPath(linear_path, dm.eef_step, dm.jump_threshold, trajectory, false);
  my_plan.trajectory_ = trajectory;
  dm.rightArm->execute(my_plan);
  ...
  // [RIGHT]: close the gripper
  gripper_msg.data = 'c';
  dm.gripper_pub_right.publish(gripper_msg);

  ros::Duration(2).sleep(); // wait for 3 sec

  // [LEFT]: open the gripper
  gripper_msg.data = 'o';
  dm.gripper_pub_left.publish(gripper_msg);
  ...
  // [RIGHT]: rotate the wrist
  dm.rightArm->setStartStateToCurrentState();
  std::vector<double> joint_values = dm.rightArm->getCurrentJointValues();
  dm.rightArm->setJointValueTarget("right_shoulder_pan_joint", joint_values[0]); //
  dm.rightArm->setJointValueTarget("right_shoulder_lift_joint", joint_values[1]); //
  dm.rightArm->setJointValueTarget("right_elbow_joint", joint_values[2]); //
  dm.rightArm->setJointValueTarget("right_wrist_1_joint", joint_values[3]); //
  dm.rightArm->setJointValueTarget("right_wrist_2_joint", joint_values[4]); //
  dm.rightArm->setJointValueTarget("right_wrist_3_joint", -85*D2R);  //
  success = (dm.rightArm->plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
  dm.rightArm->execute(my_plan);

  // [RIGHT]: move back
  sphere_r_pose = dm.rightArm->getCurrentPose(dm.EE_LINK_R).pose;
  sphere_r_pose.position.y = 0.2;
  linear_path.push_back(sphere_r_pose);
