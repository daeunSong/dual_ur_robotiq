...
//  dm.initPose();
  dm.left_drawing_initPose();

  std::vector<geometry_msgs::Pose> linear_path;

  dm.drawing_file_name = "curves_on_sphere_8.txt";
  dm.init_drawing_pose_r = dm.leftArm->getCurrentPose(dm.EE_LINK_R).pose;
  dm.init_drawing_pose_l = dm.rightArm->getCurrentPose(dm.EE_LINK_L).pose;
  dm.init_drawing_pose_r.position.x = -0.6;
  dm.init_drawing_pose_l.position.x = -0.6;
  dm.init_drawing_pose_r.position.y = 0.0;
  dm.init_drawing_pose_l.position.y = 0.0;
  dm.init_drawing_pose_r.position.z = 0.95;
  dm.init_drawing_pose_l.position.z = 0.95;
  DrawingInput drawing(dm.drawing_file_name, dm.init_drawing_pose_r, dm.init_drawing_pose_l);

  dm.rightArm->setEndEffectorLink(dm.EE_LINK_R);
  dm.leftArm->setEndEffectorLink(dm.EE_LINK_L);

  geometry_msgs::Pose sphere_l_pose = dm.leftArm->getCurrentPose(dm.EE_LINK_L).pose;
  geometry_msgs::Pose sphere_r_pose = dm.rightArm->getCurrentPose(dm.EE_LINK_R).pose;

/*
  sphere_l_pose.position.y = -0.1;
  sphere_l_pose.position.z = 0.95;
//  sphere_l_pose.orientation.x = 0.0;
//  sphere_l_pose.orientation.y = 0.0;
//  sphere_l_pose.orientation.z = 0.7071068;
//  sphere_l_pose.orientation.w = 0.7071068;
  ...
  my_plan.trajectory_ = trajectory;
  std::cin >> input;
  if (fraction == 1.0 ) dm.leftArm->execute(my_plan);
  linear_path.clear();
*/

  sphere_r_pose = dm.rightArm->getCurrentPose(dm.EE_LINK_R).pose;
  sphere_r_pose.position.x = -0.6-0.01;
  sphere_r_pose.position.y = -0.005;
  sphere_r_pose.position.z = 0.95+0.015;
  ...
  dm.leftArm->setEndEffectorLink(dm.PEN_LINK_L);

  sphere_l_pose.position.x = -0.6;
  sphere_l_pose.position.y = 0.0;
  sphere_l_pose.position.z = 0.95+0.1;
  sphere_l_pose.orientation.x = 0.0;
  sphere_l_pose.orientation.y = 0.0;
  sphere_l_pose.orientation.z = 0.0;
  sphere_l_pose.orientation.w = 1.0;
  linear_path.push_back(sphere_l_pose);
  fraction = dm.leftArm->computeCartesianPath(linear_path, dm.eef_step, dm.jump_threshold, trajectory, false);
  my_plan.trajectory_ = trajectory;
  if (fraction == 1.0 ) dm.leftArm->execute(my_plan);
...
//  dm.rightArm->setEndEffectorLink(dm.PEN_LINK_R);
//  dm.leftArm->setEndEffectorLink(dm.PEN_LINK_L);

//  sphere_r_pose = dm.rightArm->getCurrentPose(dm.PEN_LINK_R).pose;
//  sphere_r_pose.position.x = -0.5;
//  sphere_r_pose.position.y = 0.1;
//  sphere_r_pose.position.z = 0.7;
////  sphere_r_pose.orientation.x = 0.0;
////  sphere_r_pose.orientation.y = 0.0;
////  sphere_r_pose.orientation.z = 1.0;
////  sphere_r_pose.orientation.w = 0.0;
////  sphere_r_pose.orientation.x = 0.0;
////  sphere_r_pose.orientation.y = 0.0;
////  sphere_r_pose.orientation.z = -0.7071068;
////  sphere_r_pose.orientation.w = 0.7071068;
...
//  linear_path.clear();
//
//  sphere_l_pose = dm.leftArm->getCurrentPose(dm.PEN_LINK_L).pose;
//  sphere_l_pose.position.x = -0.5;
//  sphere_l_pose.position.y = -0.1;
//  sphere_l_pose.position.z = 0.87;
////  sphere_l_pose.orientation.x = 0.0;
////  sphere_l_pose.orientation.y = 0.0;
////  sphere_l_pose.orientation.z = 0.7071068;
////  sphere_l_pose.orientation.w = 0.7071068;
...
//  sphere_r_pose.position.y = -0.00;
//  sphere_r_pose.position.z = 1.0;
//  linear_path.push_back(sphere_r_pose);
//  fraction = dm.rightArm->computeCartesianPath(linear_path, dm.eef_step, dm.jump_threshold, trajectory, false);
//  my_plan.trajectory_ = trajectory;
//  std::cin >> input;
...
//
//
//  for (int i = drawing.strokes.size()-1; i >= 0; i--) {
//    Stroke stroke = drawing.strokes[i];
//    std::cout << "drawing " << str_num << " / " << drawing.strokes.size() << std::endl;
//
//    for (auto p : stroke){
//      pose_array.header.seq = str_num;
//      pose_array.poses.push_back(p);
//      dm.marker_pub.publish(pose_array);
//    }
//
//    // ready pose
//    linear_path.push_back(stroke[0]);
//    fraction = dm.leftArm->computeCartesianPath(linear_path, dm.eef_step, dm.jump_threshold, trajectory, false);
//    ready_plan.trajectory_ = trajectory;
//    if (fraction == 1.0 ) dm.leftArm->execute(ready_plan);
//    linear_path.clear();
//
//    //stroke
//    fraction = dm.leftArm->computeCartesianPath(stroke, dm.eef_step, dm.jump_threshold, trajectory, false);
//    my_plan.trajectory_ = trajectory;
//    if (fraction == 1.0) {
//        dm.trajectory_pub.publish(trajectory);
...
//  std::vector<geometry_msgs::Pose> linear_path;
//
//  // [ARMS]: move to center ready pose
//  geometry_msgs::Pose curr_pose_l = dm.leftArm->getCurrentPose(dm.EE_LINK_L).pose;
//  geometry_msgs::Pose curr_pose_r = dm.rightArm->getCurrentPose(dm.EE_LINK_R).pose;
//  geometry_msgs::Pose target_pose_l = curr_pose_l;
//  geometry_msgs::Pose target_pose_r = curr_pose_r;
//
//  target_pose_r.position.x = -0.4;
//  target_pose_r.position.y = -0.0;
//  linear_path.push_back(target_pose_r);
//
//  fraction = dm.rightArm->computeCartesianPath(linear_path, dm.eef_step, dm.jump_threshold, trajectory, false);
//  my_plan.trajectory_ = trajectory;
...
//
//  //left
//  target_pose_l.position.y = 0.0;
//  linear_path.push_back(target_pose_l);
//
//  fraction = dm.leftArm->computeCartesianPath(linear_path, dm.eef_step, dm.jump_threshold, trajectory, false);
//  my_plan.trajectory_ = trajectory;
//  dm.leftArm->execute(my_plan);
...
//  linear_path.clear();
  ...
  ros::Duration(10).sleep(); // wait for 3 sec
