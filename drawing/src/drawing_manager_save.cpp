...
//  rightArm.execute(my_plan_arm_r);
  ...
  for (int i = 0; i < dm.colors.size(); i ++)
  {
    ...
    for (auto stroke : dm.drawings[i].strokes)
    {
      ...
      std::stringstream ss; int j = 0;

      for (j = 0; j<jt.points.size(); j++)
      {
          ss << "point_index: " << j << std::endl
             << "positions: "
             << "[" << jt.points[j].positions[0]
             << "," << jt.points[j].positions[1]
             << "," << jt.points[j].positions[2]
             << "," << jt.points[j].positions[3]
             << "," << jt.points[j].positions[4]
             << "," << jt.points[j].positions[5]
             << "]" << std::endl;
      }
      std::ofstream outfile (ros::package::getPath("drawing") + "/data/trajectory/"+dm.drawing_file_name+dm.colors[i]+std::to_string(stroke_num)+".yaml",std::ios::app);
      if(!outfile.is_open())
      {
