...
class DrawingManager:
    ...
    def drawing_test_r (self):
        self.arm = dm.right_arm
        dm.set_pose_lin([-0.55, 0.00, 1.34, -0.7071068, 0.7071068, 0.0, 0.0])

        waypoints = []
        p = [-0.55, 0.00, 1.32, -0.7071068, 0.7071068, 0.0, 0.0]
        waypoints.append(copy.deepcopy(self.set_pose(p)))
        p = [-0.50, 0.00, 1.32, -0.7071068, 0.7071068, 0.0, 0.0]
        waypoints.append(copy.deepcopy(self.set_pose(p)))
        p = [-0.50, 0.00, 1.35, -0.7071068, 0.7071068, 0.0, 0.0]
        waypoints.append(copy.deepcopy(self.set_pose(p)))
        (plan, fraction) = self.arm.compute_cartesian_path(
            waypoints,  # waypoints to follow
            0.01,  # eef_step
    ...
    def drawing_test_l (self):
        self.arm = dm.left_arm
        dm.set_pose_lin([-0.55, 0.00, 1.34, 0.7071068, 0.7071068, 0.0, 0.0])
