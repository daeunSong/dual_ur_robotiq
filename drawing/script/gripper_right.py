...
FILE_NAME = 'sbr_6_path_'
STROKE_NUM = [1109, 1533]
...
class TrajectoryClient:
    ...
    def __init__(self):
        rospy.init_node("draw_right")

        self.state_sub = rospy.Subscriber('/right_arm/drawing_command', Int32, self.state_callback)
        self.state_pub = rospy.Publisher('/right_arm/drawing_state', Int32, queue_size=1)
        self.command = 0
        ...
        # gripper
        self.gripper = rospy.Publisher('/right_gripper/gripper_right', Char, queue_size=1)

        timeout = rospy.Duration(5)
        self.switch_srv = rospy.ServiceProxy(
            "/right_arm/controller_manager/switch_controller", SwitchController
        )
        self.load_srv = rospy.ServiceProxy("/right_arm/controller_manager/load_controller", LoadController)
        self.list_srv = rospy.ServiceProxy("/right_arm/controller_manager/list_controllers", ListControllers)
        try:
    ...
    def load_tool_traj (self):
        ...
        while (file_num <= self.num_tool):
            pick_traj = [] # pick
            for stage in pick:
                file_path = file_path_init + 'pick_'+ stage + '_r_'+str(file_num)+'.yaml'
            ...
            place_traj = [] # place
            for stage in place:
                file_path = file_path_init + 'place_'+ stage + '_r_'+str(file_num)+'.yaml'
                with open(file_path, 'r') as file_open:
                    loaded_plan = yaml.load(file_open)
    ...
    def load_drawing_traj (self):
        ...
        for color in self.colors:
            ...
            for i in range(STROKE_NUM[j]):
                file_path = file_path_init + self.file_name + 'r_' + color + '_' + str(i)+'.yaml'
                with open(file_path, 'r') as file_open:
                    loaded_plan = yaml.load(file_open)
...
if __name__ == "__main__":
    ...
    #     client.tool_place(i)
    #
    #     # wait for left to done drawing
    #     while client.command == 0:
    #         state.data = 0
    #         if i == client.num_drawing-1: break
